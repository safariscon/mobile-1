...
export function normalizeServiceDetail(item, index = 0) {
  ...
  const options = apiOptions.length
    ? apiOptions.map((option, optionIndex) => ({
        ...
        priceType: firstValue(option.priceType, ''),
        durationUnit: firstValue(option.durationUnit, i18n.t('serviceDetails.use')),
        duration: firstValue(option.maximumDuration, ''),
        maximumCapacity: Math.max(1, numberFrom(option.capacity, option.attributes?.quantity, availableQuantity, 1)),
        capacity: numberFrom(option.capacity, option.attributes?.quantity, 1),
        ...
        availableFrom: option.availableFrom || option.availability?.windowStartDate || '',
        availableTo: option.availableTo || option.availability?.windowEndDate || '',
        availableDays: option.availableDays || option.availability?.daysOfWeek || [],
  ...
  return {
    ...
    domain: firstValue(item.domain, item.schemaSnapshot?.domain, ''),
    subtype: firstValue(item.subtype, item.schemaSnapshot?.subtype, categorySlug),
    inventoryLabel: firstValue(item.inventoryLabel, ''),
    rooms: Array.isArray(item.rooms) ? item.rooms : [],
    nestedServices: Array.isArray(item.nestedServices) ? item.nestedServices : [],
    availability: item.availability || null,
    availabilities: Array.isArray(item.availabilities) ? item.availabilities : [],
    units: Array.isArray(item.units) && item.units.length ? item.units : options,
